...
export async function deployToPreviewEnvironment(werft: Werft, jobConfig: JobConfig) {
    ...
    exec(`kubectl --kubeconfig ${CORE_DEV_KUBECONFIG_PATH} get secret ${'preview-envs-authproviders-harvester'} --namespace=keys -o jsonpath="{.data.authProviders}" > auth-provider-secret.yml`, { silent: true })
    ...
    try {
        ...
        await installMetaCertificates(werft, jobConfig.repository.branch, 'default', PREVIEW_K3S_KUBECONFIG_PATH, vmSlices.COPY_CERT_MANAGER_RESOURCES)
...
export async function issueMetaCerts(werft: Werft, certName: string, certsNamespace: string, domain: string, slice: string) {
...
async function installMetaCertificates(werft: Werft, branch: string, destNamespace: string, destinationKubeconfig: string, slice: string) {
